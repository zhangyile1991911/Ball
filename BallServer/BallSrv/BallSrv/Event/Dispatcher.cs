...
public class MDispatcher
{
    ...
    private class Receiver<T,P> : IReceiver
    {
        private Action<T, P> mReceiver;

        public Receiver(Action<T,P> receiver)
        {
            mReceiver = receiver;
        }
        ...
        public void Invoke(T data,P peer)
        {
            if (mReceiver != null)
            {
                mReceiver(data,peer);
        ...
        public bool Equals(IReceiver receiver)
        {
            ...
            Receiver<T,P> o = receiver as Receiver<T, P>;
            if (o == null)
            {
                return false;
            }
        ...
        public bool Equals(Action<T, P> receiver)
        {
            return mReceiver == receiver;
        }
    }
    ...
    public bool RegisterReceiver<T,P>(int id, Action<T,P> action)
    {
        if (action == null) return false;
        ...
        Receiver<T, P> receiver = new Receiver<T, P>(action);
    ...
    public void UnRegisterReceiver<T,P>(int id, Action<T,P> action)
    {
        if (mReceiverDic.ContainsKey(id))
        {
            for (int i = mReceiverDic[id].Count - 1; i >= 0; --i)
    ...
    public void Dispatch<T,P>(int id, T data,P peer)
    {
        if (mReceiverDic.ContainsKey(id))
        {
            for (int i = 0; i < mReceiverDic[id].Count; ++i)
